...
function PatchedPaginatedChoiceState(name, next, question, choices, error,
                                     handlers, page_opts, options) {
    var self = this;
    PaginatedChoiceState.call(self, name, next, question, choices, error,
                              handlers, page_opts, options);

    self.translate = function(i18n) {
        // when used with PaginatedChoiceState this causes an [object Object]
        // to be renderd instead of the "More" and "Back options".
        // This hack prevents that from happening
    };
}
...
function MamaSMS() {
    ...
    self.add_state(new PatchedPaginatedChoiceState(
        'expected_month',
        function(choice) {
    ...
    self.add_creator('too_old',
        self.make_fake_exit_menu(
            'optstatus',
            ("MAMA SMSs are aimed at mothers of younger babies. You can " +
