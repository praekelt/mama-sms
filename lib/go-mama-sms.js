...
function MamaSMS() {
    ...
    self.add_state(new EndState(
        'get_tested',
        _.gettext("Don't wait! The first pregnancy check-up must happen as soon as " +
         "you know. Do the test as soon as possible at any clinic, or get " +
         "one at a pharmacy. Stay well."),
        'optstatus'
    ));

    self.add_state(new EndState(
        'go_to_clinic',
        _.gettext("To sign up, we need to know which month. Please go to the clinic to " +
    ...
    self.add_state(new EndState(
        'too_old',
        _.gettext("Sorry, MAMA SMSs are aimed at mothers of younger babies. " +
    ...
    self.add_state(new EndState(
        "cancel",
        _.gettext("To receive MAMA SMSs you will need to answer the 4 questions."),
        "optstatus"
    ));
    ...
    self.add_creator('end', function(state_name, im) {
        ...
        p.add_callback(function(result) {
            ...
            var fields = {
                ...
                'mama-sms-hiv-messages': im.get_user_answer('hiv_messages'),
